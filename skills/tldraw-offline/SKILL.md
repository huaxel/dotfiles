...
The default server is `http://localhost:7236`. If that port is not active, read the `port` from `/Users/juanbenjumea/Library/Application Support/tldraw/server.json`.
...
```bash
PORT=$(node -p 'JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")).port' '/Users/juanbenjumea/Library/Application Support/tldraw/server.json'); TOKEN=$(node -p 'JSON.parse(require("fs").readFileSync(process.argv[1],"utf8")).token' '/Users/juanbenjumea/Library/Application Support/tldraw/server.json')
