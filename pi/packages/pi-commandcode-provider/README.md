...
Open `/model` and select one of the models provided by Command Code. Pi starts from the last cached catalog so startup is immediate, then refreshes it in the background. Use `/commandcode-refresh` when you need the latest catalog before selecting a newly added model.
...
Pi loads the cached catalog first and refreshes it in the background. If the endpoint is unavailable, the cached catalog remains active. On a first offline start without a cache, pi still loads, but Command Code models remain unavailable until the connection is restored and `/commandcode-refresh` succeeds.
