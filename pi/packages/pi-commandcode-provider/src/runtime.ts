...
export interface CommandCodeRuntimeOptions<TProviderConfig> {
  ...
  loadCachedModels?: () => Promise<LoadCommandCodeModelsResult>
...
export class CommandCodeRuntime<TProviderConfig, TContext extends CommandCodeCommandContext> {
  ...
  async initialize(): Promise<void> {
    ...
    // A cached catalog is enough to make the provider usable immediately.
    // Refreshing the remote catalog must not hold the whole Pi startup path.
    if (this.options.loadCachedModels) {
      const cached = await this.options.loadCachedModels()
      if (cached.models.length > 0) {
        this.pi.registerProvider("commandcode", this.options.createProviderConfig(cached.models))
        ...
        if (!process.env.PI_OFFLINE) void this.refresh()
        return
      }
    }
