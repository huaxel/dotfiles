...
/** Load the last known catalog without making a network request. */
export async function loadCommandCodeModelsCache(
  cachePath: string,
): Promise<LoadCommandCodeModelsResult> {
  try {
    return {
      models: await readCommandCodeModelsCache(cachePath),
      source: "cache",
    }
  } catch (error) {
    return {
      models: [],
      source: "empty",
      warning: `No valid cached Command Code model catalog at ${cachePath}: ${errorMessage(error)}`,
    }
  }
}
