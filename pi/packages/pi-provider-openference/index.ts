...
export default async function (pi: ExtensionAPI) {
  ...
  pi.registerCommand("openference-refresh", {
    ...
    handler: async (_args, ctx) => {
      ...
      const models = await fetchModels(resolveStartupApiKey());
      if (models.length === 0) throw new Error("Openference returned no models");
      registerOpenferenceProvider(pi, models);
      ctx.ui.notify(`Openference model catalog refreshed (${models.length} models).`, "info");
    },
  });
