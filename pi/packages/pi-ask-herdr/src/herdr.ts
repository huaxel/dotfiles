...
/**
 * Herdr metadata is display-only. Failures (e.g. a stale `HERDR_PANE_ID`
 * pointing at a pane that was closed and recreated) are non-fatal and must
 * not spam the agent's terminal. Set `PI_ASK_HERDR_DEBUG=1` to see them.
 */
const DEBUG = process.env.PI_ASK_HERDR_DEBUG === "1";
function warn(prefix: string, err: unknown): void {
	if (!DEBUG) return;
...
export function setHerdrBlocked(pi: ExtensionAPI, active: boolean, label?: string) {
	try {
		...
		warn("[pi-ask-herdr] failed to emit herdr:blocked:", err);
	}
}

/**
 * Send a single JSON-RPC-style request to the Herdr Unix socket and return
 * the matching result. Herdr speaks newline-delimited JSON. The server keeps
 * connections open, so resolve as soon as the matching response arrives
	...
	return new Promise<T | undefined>((resolve, reject) => {
		...
		const handleLine = (line: string) => {
			...
			try {
				...
				if (msg.id !== id) return;
				if (msg.error) {
					finish(undefined, new Error(msg.error.message ?? "herdr request failed"));
				} else {
					finish(msg.result);
				}
			} catch {
				// Ignore malformed or unrelated lines; the timeout remains active.
			}
		};

		const timer = setTimeout(() => {
			finish(undefined, new Error("herdr socket timeout"));
		}, 1000);
		timer.unref?.();
		...
		client.on("error", (err) => finish(undefined, err));
	});
}
...
export async function reportAskMetadata(questionTexts: string[]): Promise<void> {
	try {
		...
		warn("[pi-ask-herdr] pane.report_metadata failed:", err);
	}
}
...
export async function clearAskMetadata(): Promise<void> {
	try {
		...
		warn("[pi-ask-herdr] failed to clear pane metadata:", err);
	}
}
