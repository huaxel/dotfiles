...
- ~~Auto-continue after account switch (safe queued user prompt)~~ — **implemented 2026-08-02** (spike + prototype):
  ...
  - Smoke test: `/opencode-autocontinue-test` arms the flag, `/opencode-autocontinue-test status` shows state. Then send any prompt — the following `agent_settled` should queue the retry turn (check `pi/agent/opencode-go-failover.log` for `auto-continue turn=…`).
