...
This repository carries a local fork of `@ogulcancelik/pi-auto-permissions` so the
OpenCode Go reviewer session-header fix remains stable across npm updates. The
runtime loads this directory from `pi/agent/settings.json`.
