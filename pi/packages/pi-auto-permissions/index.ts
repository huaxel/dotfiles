...
import { findGates, type Gate } from "./gates.ts";
...
export default function autoPermissionsExtension(pi: ExtensionAPI) {
  ...
  async function reviewCommand(
    ...
    try {
      ...
      // completeSimple bypasses Pi's agent-level header transform, so inject
      // OpenCode Go's required routing headers explicitly for reviewer calls.
      const response = await completeSimple(
        model,
        { systemPrompt, messages },
        {
          apiKey: auth.apiKey,
          headers: buildReviewerHeaders(model.provider, sessionId, auth.headers),
          env: auth.env,
          signal: reviewSignal,
          reasoning: config.reviewer?.reasoningEffort ?? "low",
          sessionId,
