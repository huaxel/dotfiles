...
- Cron: `0 3 * * *` — hardlink-incremental snapshots (`/data/backups/pi/YYYY-MM-DDTHHMMSS`), 30-day retention, Immich `pg_dumpall`, and a K3s etcd restore bundle with server token/config. Failed snapshots are removed automatically; `latest` advances atomically only after all primary stages succeed.
- The 1 TB acerpepe target intentionally excludes `~/media/{movies,series,downloads}`, CI runner trees, and language/package caches from nightly home snapshots. Kingston2TB remains the media archive/source; acerpepe backs up Docker configs, application data, databases, and K3s recovery state only.
...
- `smartd` now monitors framearch, acerpepe, and liedelpi. All drives currently pass SMART; liedelpi’s NVMe has historical critical-temperature time, so keep its cooling/temperature under review.
...
- Restore: rsync from `/data/backups/pi/latest/` back to the Pi. `k3s-restore.tar` contains a fresh compressed etcd snapshot plus `/var/lib/rancher/k3s/server/token` and optional `/etc/rancher/k3s/config.yaml`; follow the matching K3s snapshot-restore procedure before restoring application data.
...
6. **Canonical media runtime:** Jellyfin, Sonarr, Radarr, Prowlarr, qBittorrent, and Jellyseerr run in Docker Compose (`~/docker`, project `docker`) and share the Docker config paths. Their duplicate k8s Deployments are intentionally scaled to zero. The `media` Services have no selectors and use manually managed EndpointSlices to fixed `media-net` addresses (`172.18.0.5`, `.10`, `.11`, `.12`, `.13`, `.14`); compose pins those addresses so recreation does not break NPM. Radarr/Jellyfin use `.13`/`.14` because Immich dynamically occupied the old `.2`/`.4` addresses and prevented both media containers from starting. Prowlarr has enabled full-sync application connections to Sonarr and Radarr; indexers remain a manual setup step. Media web ports bind to `192.168.1.138` only; qBittorrent's torrent port 6881 remains published on all interfaces.
7. **Dual-runtime services (Immich, Portainer, IPTVnator):** Immich, Portainer, and IPTVnator each run on both Docker Compose and k3s — the Docker versions are the legacy runtime, k3s is the canonical runtime. Web ports are LAN-bound on the Docker containers (`192.168.1.138`); k3s Services were converted from NodePort to ClusterIP to remove host-level exposure. NPM proxy hosts route to Docker for Immich/Portainer (`192.168.1.138:2283`, `192.168.1.138:9000`) and to k3s for IPTVnator (`iptvnator.media.svc.cluster.local:8185`). The iptvnator Docker container was stopped and removed from the compose aggregator in favor of the k3s runtime only. Hardening committed 2026-08-25.
8. Docker containers on `docker0` can show `linkdown` (cosmetic; custom bridges unaffected).
...
```
Then verify: `curl -s http://127.0.0.1:2283/api/server/ping` (Immich 200), container egress (`docker run --rm alpine:3.20 wget -q -O /dev/null https://registry.npmjs.org/pnpm`), and `.home` proxy routes (`curl -H 'Host: sonarr.home' http://192.168.1.138/`, `curl -H 'Host: portainer.home' http://192.168.1.138/`, `curl -H 'Host: iptvnator.home' http://192.168.1.138/`). Do not scale the duplicate media Deployments back up; NPM reaches the pinned Docker backends through the selectorless `media` Services. Note: `portainer.home` uses the k3s Service as forward target; the Docker Portainer container requires a direct `Host` header that matches its origin — if proxying returns 307, re-point the NPM proxy at the Docker container's `192.168.1.138:9000` instead of the k3s Service.
