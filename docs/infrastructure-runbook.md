...
- Framearch critical data is backed up daily by the user timer `framearch-restic.timer` to the encrypted Restic repository `/data/backups/framearch-restic`, with a second encrypted mirror at `/mnt/wd-blue/backups/framearch-restic`. It includes dotfiles, projects, documents, configs, SSH, and Pi state; caches, model data, build dependencies, and runner state are excluded. The password file is `~/.config/restic/framearch-acerpepe.password` and must also be stored in a password manager.
